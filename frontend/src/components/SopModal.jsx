...
import { X, Clock, ChefHat, Loader2 } from 'lucide-react';
                ...
                <div className="p-6 overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-2 text-emerald-600 bg-emerald-50 w-fit px-3 py-1.5 rounded-lg text-sm font-semibold">
                                <Clock className="w-4 h-4" />
                                เวลามาตรฐาน: {guide.standardtime} วินาที
                            </div>

                            <div>
                                <h4 className="font-bold text-slate-800 mb-3 text-lg">ขั้นตอนการทำ:</h4>
                                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                                    <ol className="list-decimal pl-4 space-y-3 text-slate-700 font-medium leading-relaxed">
                                        {guide.instruction.split('\n').map((step, i) => {
                                            // Remove leading numbers (e.g., "1. ") if they exist in DB so we can use native ol styling
                                            const cleanStep = step.replace(/^\d+\.\s*/, '');
                                            return <li key={i} className="pl-2 marker:text-emerald-500">{cleanStep}</li>
                                        })}
                                    </ol>
                                </div>
                            </div>
